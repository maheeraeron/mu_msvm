  ...
  )
{
  ...
  GicInterrupt = (UINT32)ArmGicV3AcknowledgeInterrupt ();
  ...
  )
{
  ...
  mGicRedistributorsBase = (UINT32)PcdGet64 (PcdGicRedistributorsBase);
  ...
  if (FeaturePcdGet (PcdArmGicV3WithV2Legacy)) {
    ...
    if (CpuTarget != 0) {
      ...
      for (Index = 8; Index < (mGicNumInterrupts / 4); Index++) {
        MmioWrite32 (mGicDistributorBase + ARM_GIC_ICDIPTR + (Index * 4), (UINT32)CpuTarget);
      }
    }
  } else {
    MpId = ArmReadMpidr ();
    ...
    for (Index = 0; Index < (mGicNumInterrupts - 32); Index++) {
      MmioWrite32 (mGicDistributorBase + ARM_GICD_IROUTER + (Index * 8), (UINT32)(CpuTarget | ARM_GICD_IROUTER_IRM));
    }
  }
