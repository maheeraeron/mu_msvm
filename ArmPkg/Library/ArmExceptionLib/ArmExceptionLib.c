  ...
  )
{
  ...
  // Confirm start of exception vector table is aligned
  ASSERT(((UINTN)ExceptionHandlersStart & gExceptionVectorAlignmentMask) == 0);

  // get current VBAR in order examine the current EL sync exception code

  UINTN ExistingSyncExVectorAddress = ArmReadVBar() + (128 * 4); // fifth vector

  // Is the existing vector an unconditional branch to offset 0?
  // If not hook existing vectors.

  if (*((UINT32*)((UINTN)ExistingSyncExVectorAddress)) != 0x14000000)
  {
    //
    // Special AARCH64 code for the Microsoft UEFI boot debugger.
    // - Leave the VBAR pointing at the original boot debugger exception vectors.
    // - Modify the boot debugger IRQ exception vector to branch to this module's IRQ vector.
    //   Due to distance to branch x0 must be used so it is pushed on the stack.
    // - Modify this module's IRQ vector to restore the stack.
    //

    #define AARCH64_SUB_SP_8            0xd10023ff     // sub sp, sp, #8
    #define AARCH64_STR_X0_SP           0xf90003e0     // str x0, [sp]
    #define AARCH64_MOVZ_X0_HIGH16      0xd2a00000     // movz x0, <imm16>, lsl #16
    #define AARCH64_MOVK_X0_LOW16       0xf2800000     // movk x0, <imm16>
    #define AARCH64_BR_X0               0xd61f0000     // br x0
    #define AARCH64_MOVZK_IMM_SHIFT     0x5            // shift of imm16

    #define AARCH64_LDR_X0_SP           0xf94003e0     // ldr x0, [sp]
    #define AARCH64_ADD_SP_8            0x910023ff     // add sp, sp, #8

    UINTN DebuggerIrqVectorAddress;
    UINTN NewIrqVectorAddress;

    DebuggerIrqVectorAddress = ArmReadVBar(); // get base of existing vectors
    DEBUG((DEBUG_VERBOSE, "Debug VBar   %p\n", DebuggerIrqVectorAddress));
    DebuggerIrqVectorAddress += (128 * 5);    // offset to specific handler - sixth vector
    DEBUG((DEBUG_VERBOSE, "Debug Vector %p\n", DebuggerIrqVectorAddress));
    NewIrqVectorAddress = (UINTN)ExceptionHandlersStart; // base of new vectors
    DEBUG((DEBUG_VERBOSE, "New V table  %p\n", NewIrqVectorAddress));
    NewIrqVectorAddress += (128 * 5);
    DEBUG((DEBUG_VERBOSE, "New Vector   %p\n", NewIrqVectorAddress));

    // replace debugger IRQ exception handler

    *((UINT32*)((UINTN)DebuggerIrqVectorAddress))       = AARCH64_SUB_SP_8;
    *((UINT32*)((UINTN)DebuggerIrqVectorAddress + 4))   = AARCH64_STR_X0_SP;
    *((UINT32*)((UINTN)DebuggerIrqVectorAddress + 8))   = AARCH64_MOVZ_X0_HIGH16 |
      (((NewIrqVectorAddress >> 16) & 0xFFFF) << AARCH64_MOVZK_IMM_SHIFT);
    *((UINT32*)((UINTN)DebuggerIrqVectorAddress + 12))  = AARCH64_MOVK_X0_LOW16 |
      ((NewIrqVectorAddress & 0xFFFF) << AARCH64_MOVZK_IMM_SHIFT);
    *((UINT32*)((UINTN)DebuggerIrqVectorAddress + 16))  = AARCH64_BR_X0;

    // flush new code through data cache

    WriteBackDataCacheRange((VOID*)DebuggerIrqVectorAddress, 128);

    // replace the first nop instruction in

    *((UINT32*)((UINTN)NewIrqVectorAddress))            = AARCH64_LDR_X0_SP;
    *((UINT32*)((UINTN)NewIrqVectorAddress + 4))        = AARCH64_ADD_SP_8;

    // flush new code through data cache

    WriteBackDataCacheRange((VOID*)NewIrqVectorAddress, 128);
