...
/**
  KdDebug registration notification callback
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    mIsKdAvailable = TRUE;
    mBS->CloseEvent (mKdDebugRegisterEvent);
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    Status = mBS->CreateEvent (
                    ...
                    KdDebugHandlerCallback,
                    NULL,
                    &mKdDebugRegisterEvent
                    );

    //
    // Setup callback in case KD is installed later
    //
    if (!EFI_ERROR (Status)) {
      mBS->RegisterProtocolNotify (
             &gKdDebugPrintGuid,
             mKdDebugRegisterEvent,
             &Registration
             );
  ...
  )
{
  ...
  if (mIsKdAvailable && mKdDebugPrint && mKdDebugPrint->IsConnected ()) {
    DebugPrintFunction = KdDebugPrint;
  } else if (mIsBSAvailable && mIsRscAvailable) {
  ...
  )
{
  ...
  if (mIsKdAvailable && mKdDebugPrint && mKdDebugPrint->IsConnected ()) {
    DebugAssertFunction = KdDebugAssert;
  } else if (mIsBSAvailable && mIsRscAvailable) {
