  ...
  )
{
  ...
  // MS_HYP_CHANGE - mMasterFrameWidth is 0 due to not having a sidebar, because there are no UI controls.
  UINT32 OrigX              = (mMasterFrameWidth == 0) ? 
                                ((mGop->Mode->Info->HorizontalResolution * (FP_TBAR_MSLOGO_X_PERCENT * 2)) / 100) : 
                                (mMasterFrameWidth + ((mMasterFrameWidth * FP_FCANVAS_BORDER_PAD_WIDTH_PERCENT) / 100));
