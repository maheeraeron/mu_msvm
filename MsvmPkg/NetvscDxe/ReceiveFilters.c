...
SnpRecvFilterEnable(
    ...
    _In_ UINTN           MCastAddressCount,
    _In_ EFI_MAC_ADDRESS *MCastAddressList
    )
/**

Routine Description:

    Enable the receive filters.
...
Arguments:
    ...
    MCastAddressCount  Multicast address count for a new multicast address
                             list.
...
Return Values:
    ...
    EFI_SUCCESS           The receive filters were updated.

    EFI_INVALID_PARAMETER Invalid command.

    EFI_UNSUPPORTED       Command is not supported.

    EFI_DEVICE_ERROR      Fail to update filters.
...
SnpRecvFilterDisable(
    _In_ SNP_DRIVER *Snp,
    _In_ UINT32     DisableFlags,
    _In_ BOOLEAN    ResetMCastList
    )
/**

Routine Description:

    Disable the receive filters.
...
Arguments:
    ...
    ResetMCastList  Boolean flag to reset/delete the multicast filter 
                          list.
...
Arguments:
    ...
    Enable           A bit mask of receive filters to enable on the network
                          interface.

    Disable          A bit mask of receive filters to disable on the network
    ...
    MCastFilterCnt   Number of multicast HW MAC addresses in the new MCastFilter
    ...
    MCastFilter      A pointer to a list of new multicast receive filter HW
