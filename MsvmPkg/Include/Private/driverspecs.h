...
#if _SAL_enable_paging_
    // Paging information:
    // _Is_pageable_(n)  _Is_discardable_(n)
    //    Is the function itself paged or discardable.
    //       The analysis doesn't care, but it's more easily read as 
    //       two annotations.
    //    _Analysis_assume_pageable_(n)
    //    _Analysis_assume_discardable(n)
    //       Like the above, but internal only (not part of the contract)
    //    _Analysys_assume_locked_section(n)
    //    _Analysis_assume_unlocked_section(n)
    //       A call will be made to <n>, and we should assume it is [not]
    //       locked in memory.

    __ANNOTATION(SAL_functionUsesPaged(__In_impl_ char *););

    #define _Is_pageable_(name)                                           \
        _SAL2_Source_(_Is_pageable_, (name), __static_context(            \
        SAL_immediate, _Pre_ _SA_annotes1(SAL_functionUsesPaged, name)))
    #define _Is_discardable_(name)                                        \
        _SAL2_Source_(_Is_discardable_, (name), __static_context(         \
        SAL_immediate, _Pre_ _SA_annotes1(SAL_functionUsesPaged, name)))

    __ANNOTATION(SAL_callerCouldPage(__In_impl_ char *););

    // The caller could be paged if we chose to, but we don't want to
    // annotate it that way for now.
    #define _Analysis_assume_pageable_(name)                              \
        typedef _SAL2_Source_(_Analysis_assume_pageable_, (name),         \
            _Pre_ _SA_annotes1(SAL_callerCouldPage, name))                \
		__GENSYM(__prefast_analysis_mode_flag);
    #define _Analysis_assume_discardable_(name)                           \
        typedef _SAL2_Source_(_Analysis_assume_pageable_, (name),         \
            _Pre_ _SA_annotes1(SAL_callerCouldPage, name))                \
		__GENSYM(__prefast_analysis_mode_flag);
