...
EFI_DUMP_BUFFER     EfiKdDumpBuffer = {0};
MemoryDumpType      EfiKdDumpType   = MemoryDumpDisabled;
    ...
    )
{
    ...
    EfiDumpBufferInitialize(&EfiKdDumpBuffer, dumpBuffer, EFI_DUMP_BUFFER_SIZE);

    //
    // This could be configured via the BIOS config hob
    //
    EfiKdDumpType = MemoryDumpTriage;
...
{
    PCONTEXT            context   = &EfiKdPrcb->ProcessorState.ContextFrame;
    PKPROCESSOR_STATE   procState = &EfiKdPrcb->ProcessorState;
