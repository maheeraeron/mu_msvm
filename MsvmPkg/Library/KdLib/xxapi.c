...
#error( "unknown target machine" );
    ...
    m->u.GetVersion64.PsLoadedModuleList = (UINT64)(UINT_PTR)&EfiKdModuleList;
    m->u.GetVersion64.KernBase = (UINT64)(UINT_PTR)EfiKdModuleDataTableEntry->DllBase;
    ...
    m->u.GetVersion64.DebuggerDataList = (UINTN)&EfiKdDebuggerDataListHead;
    ...
    EfiKdSendPacket(PACKET_TYPE_KD_STATE_MANIPULATE, &messageHeader, NULL);
    return;
}
...
--*/
{
    ...
    EfiKdCopyMemory(AdditionalData->Buffer, (PCHAR)Context, sizeof(CONTEXT));
...
--*/
{
    ...
    m->ReturnStatus = STATUS_SUCCESS;
    EfiKdCopyMemory((PCHAR)Context, AdditionalData->Buffer, sizeof(CONTEXT));
...
--*/
{
    ...
    AdditionalData->Length = (UINT16)EfiKdMoveMemory(
...
--*/
{
    ...
    length = EfiKdMoveMemory((PCHAR)(UINT_PTR)m->u.WriteMemory.TargetBaseAddress,
                             (PCHAR)AdditionalData->Buffer,
...
--*/
{
    ...
    a->BreakPointHandle = EfiKdAddBreakpoint(a->BreakPointAddress);
...
--*/
{
    ...
    if (EfiKdDeleteBreakpoint(a->BreakPointHandle)) {
        m->ReturnStatus = STATUS_SUCCESS;
...
--*/
{
       ...
       PAGE_ALIGN((PUCHAR)((UINT_PTR)a->TargetBaseAddress) + length))
    {
        ...
        virtualAddress=EfiKdTranslatePhysicalAddress(source);
        ...
        {
            AdditionalData->Length = (UINT16)EfiKdMoveMemory(destination,
                                                             virtualAddress,
                                                             bytesLeft);

            bytesLeft = bytesLeft - AdditionalData->Length;
            EfiKdUnmapVirtualAddress(virtualAddress);
        }
    }
    else
    {
            ...
            while(bytesLeft > 0)
            {
                ...
                else
                {
                    ...
                    AdditionalData->Length = AdditionalData->Length +
                        (UINT16)EfiKdMoveMemory(destination,
                                                virtualAddress,
                                                numberBytes);
...
--*/
{
       ...
       PAGE_ALIGN(destination+bytesLeft))
    {
        ...
        virtualAddress = EfiKdTranslatePhysicalAddress(destination);
        if (virtualAddress == NULL)
        {
            Length = 0;
        }
        ...
        {
            Length = (UINT16)EfiKdMoveMemory(virtualAddress,
                                             source,
                                             bytesLeft);
...
--*/
{
    ...
    for (i = 0; i < breakPointCount; i += 1, b += 1)
    {
        ...
        {
            if (!EfiKdDeleteBreakpoint(b->BreakPointHandle))
            {
                m->ReturnStatus = STATUS_UNSUCCESSFUL;
            }
    ...
    for (i = 0; i < breakPointCount; i += 1, b += 1)
    {
        ...
        {
            b->BreakPointHandle = EfiKdAddBreakpoint( b->BreakPointAddress);
            if (!b->BreakPointHandle)
            {
                m->ReturnStatus = STATUS_UNSUCCESSFUL;
            }
    ...
    EfiKdMoveMemory(AdditionalData->Buffer,
                    (PCHAR)bpBuf,
                    breakPointCount * sizeof(DBGKD_WRITE_BREAKPOINT64));
