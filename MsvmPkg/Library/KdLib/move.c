...
--*/
{
    ...
    if (Length > EFI_KD_MESSAGE_BUFFER_SIZE) 
    {
        Length = EFI_KD_MESSAGE_BUFFER_SIZE;
    }
    ...
    while (((UINT_PTR)Source & 3) && (Length > 0)) 
    {
        ...
        Address1 = EfiKdWriteCheck((PVOID)Destination);
        Address2 = EfiKdReadCheck((PVOID)Source);
...
Routine Description:
    ...
    N.B. Unlike EfiKdMoveMemory, this routine does NOT check for accessability
