...
--*/
{
    ...
    {
        Status = EfiKdReceivePacket(PACKET_TYPE_KD_POLL_BREAKIN,
                                    NULL,
                                    NULL,
                                    NULL);

        if (Status == EFI_KD_PACKET_RECEIVED) 
        {
            BreakIn = TRUE;
            EfiKdControlCPressed = TRUE;
        }

        if (EfiKdControlCPending != FALSE) 
        {
            EfiKdControlCPressed = TRUE;
            BreakIn = TRUE;
            EfiKdControlCPending = FALSE;
        }
    }
