...
{
    //
    // The hypervisor does not have a mechanism by which it can poll the
    // power button, so this is a NOP for the hypervisor.
    //
...
--*/
{
    ...
    mcr = MC_DTRRTS;
    CppPortWriteRegister8(Port, COM_MCR, mcr);

    ier = 0;
    CppPortWriteRegister8(Port, COM_IEN, ier);

    fcr = SERIAL_ENABLE_FIFO;
    CppPortWriteRegister8(Port, COM_FCR, fcr);

    Port->IrqIden = CppPortReadRegister8(Port, COM_IDN);
}
