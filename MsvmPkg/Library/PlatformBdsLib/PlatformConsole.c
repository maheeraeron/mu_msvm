...
VOID
PlatformConsoleWaitForKey(
    _Out_opt_   EFI_INPUT_KEY       *Key
    )
/*++
...
Arguments:
    ...
    Key     Optionally returns the key that was pressed.
...
--*/
{
    ...
    UINT32  *entryNumber = ((UINT32 *)Context);
...
--*/
{
    ...
    PlatformStringPrintById(STRING_TOKEN(STR_BOOT_SUMMARY));
    ...
    //
    // Display the user prompt if desired
    //
    if (PromptId != -1)
    {
        PlatformStringPrintById(PromptId);
    }

    PlatformConsoleWaitForKey(NULL);
}
...
VOID
PlatformConsoleShowHeader(
    _In_    EFI_GRAPHICS_OUTPUT_PROTOCOL    *GraphicsOutput,
    _Out_   UINTN                           *HeaderHeight
    )
/*++

Routine Description:

    Shows the platform console header

Arguments:

    GraphicsOutput      GOP to use for display

    HeaderHeight        Returns the actual height of the header in pixels.
...
--*/
{
    ...
    UINTN imageDataSize;
    UINTN bltSize;
    UINTN headerHeight = 0;
    EFI_GRAPHICS_OUTPUT_BLT_PIXEL lineColor = {0xff, 0xff, 0xff, 0x00};
    EFI_STATUS status;

    if (mLogoBlt == NULL)
    {
        //
        // Decode and cache the logo the first time.
        //
        status = GetSectionFromAnyFv(PcdGetPtr(PcdLogoFile),
                        EFI_SECTION_RAW,
                        0,
                        &imageData,
                        &imageDataSize);
        ...
        status = ConvertBmpToGopBlt(imageData,
                          imageDataSize,
                          (VOID **) &mLogoBlt,
                          &bltSize,
                          &mImageHeight,
    ...
    GraphicsOutput->Blt(GraphicsOutput,
                ...
                0,  // Source X
                0,  // Source Y
                HEADER_LOGO_H_PADDING,  // Destination X
                HEADER_LOGO_V_PADDING,  // Destination Y
                mImageWidth,
                mImageHeight,
                mImageWidth * sizeof (EFI_GRAPHICS_OUTPUT_BLT_PIXEL));
    ...
    GraphicsOutput->Blt(GraphicsOutput,
                ...
                0,  // Source X - Unused
                0,  // Source Y - Unused
                HEADER_LINE_H_PADDING,                                        // Destination X
                mImageHeight + HEADER_LINE_V_PADDING + HEADER_LOGO_V_PADDING, // Destination Y
                GraphicsOutput->Mode->Info->HorizontalResolution - (HEADER_LINE_H_PADDING * 2),
                HEADER_LINE_WIDTH,
                0);

    headerHeight = mImageHeight + HEADER_LOGO_V_PADDING + HEADER_LINE_V_PADDING + HEADER_LINE_WIDTH;
...
Routine Description:
    ...
    Saves the current console mode and shows the platform console.
...
--*/
{
    ...
    {
        //
        // Save current console mode before switching to the
        // mode used for the platform console.
        //

        gST->ConOut->SetMode(gST->ConOut, PcdGet16(PcdPlatformConsoleMode));

        PlatformConsoleShowHeader(graphicsOutput, &headerHeight);
    }
...
Routine Description:
    ...
    Hide the platform console and restores the previously
    saved console mode.
