...
--*/
{
    ...
    moduleNameLength = BdGetModuleName(ImageContext, &moduleName);
    ...
    //   struct.  If this changes BdAddUnloadedModuleInfo will also need to change.
    //
    ...
    *Module = NULL;
    ...
    while (entry != ModuleListHead)
    {
        ...
        if (module->DllBase == ImageBase)
        {
            ...
            // BdModuleDataTableEntry
            //
            if (entry == (PLIST_ENTRY)BdModuleDataTableEntry)
            {
                ASSERT(FALSE);
...
--*/
{
    ...
    if (BdLastUnloadedModule >= BD_MAX_UNLOADED_MODULES)
    {
        BdLastUnloadedModule = 0;
    }

    pUnloadEntry = &BdUnloadedModules[BdLastUnloadedModule];
...
--*/
{
    CHAR8 buffer[BD_MAXIMUM_FILENAME_SIZE];
...
--*/
{
    ...
    status = BdAddModuleInfo(&BdModuleList,
                                ImageContext,
                                &entry);
    if (EFI_ERROR(status))
    {
    ...
    //
    // Setup value of BdModuleTableEntry.
    ...
    // in DxeMain).
    //
    if (BdModuleDataTableEntry == NULL)
    {
        BdModuleDataTableEntry = (PLDR_DATA_TABLE_ENTRY)BdModuleList.ForwardLink;
        BdDebuggerDataBlock.KernBase = ImageContext->ImageAddress;
    }

    BdImageNotification(entry, BREAKPOINT_LOAD_SYMBOLS);
}
...
--*/
{
    ...
    status = BdFindAndRemoveModuleInfo(&BdModuleList,
    ...
    BdAddUnloadedModuleInfo(module);
    BdImageNotification(module, BREAKPOINT_UNLOAD_SYMBOLS);
}
