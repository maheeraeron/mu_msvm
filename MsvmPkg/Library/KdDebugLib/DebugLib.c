...
Abstract:
    ...
    This module implements the UEFI debug library for the KD interface.
...
UINT32 EfiKdDebugPrintComponentMask = 0;
EFI_KD_DEBUG_TABLE *gKdDebugTable = NULL;

// ------------------------------------------------------------------ Functions

RETURN_STATUS
EFIAPI
KdDebugLibConstructor (
    VOID
    )
{
    //
    // KdDebugLib depends on the debug trap handlers being installed. This operation is
    // complete by the time the constructor is called.
    //
    // Note that a NULL return value indicates that the debugger is not
    // available.
    //
    gKdDebugTable = DebugService2(0, 0, BREAKPOINT_GET_TABLE);
    return 0;
}
...
--*/
{
    ...
    if (gKdDebugTable == NULL ||
        *gKdDebugTable->DebuggerNotPresent ||
        ((EfiKdDebugPrintComponentMask | *gKdDebugTable->DebuggerMask) & ErrorLevel) == 0)
    {
        return;
    }
...
--*/
{
    ...
    if (gKdDebugTable != NULL &&
        *gKdDebugTable->DebuggerNotPresent == FALSE)
    {
        __int2c();
    }
}
...
{
    if ((gKdDebugTable != NULL) && 
        (gKdDebugTable->PollBreakIn() != FALSE))
    {
        DebugBreakPointWithStatus(DBG_STATUS_CONTROL_C);
    }
}
...
--*/
{
    if (gKdDebugTable != NULL)
    {
        gKdDebugTable->BreakPointWithStatus(Status);
    }
}
