...
.NOTES
    ...
    1) For use on a build server; pushes new hyperv.uefi.test vpack when there is a new
       commit in SM_UDK.
#>
...
function Get-VPackConfig
{
    ...
    $vpackConfig = Get-Content -Path ($Workspace + "\msvmpkg\testing_vpack.json") | ConvertFrom-Json
...
try
{
    ...
    # Overwriting EDK2 config files with command line arguments
    if($PSBoundParameters.ContainsKey('BuildTarget'))
    {
        $edk2Config.TARGET = $BuildTarget
    }
    if($PSBoundParameters.ContainsKey('OutputDirectory'))
    {
        $edk2Config.OUTPUT_DIRECTORY = $OutputDirectory
    }
    if($PSBoundParameters.ContainsKey('ToolChainTag'))
    {
        $edk2Config.TOOL_CHAIN_TAG = $ToolChainTag
    }
