  ...
  )
{
          ...
          if (DevicePathType (DevicePathNode) == MEDIA_DEVICE_PATH &&
            ...
            if (((HARDDRIVE_DEVICE_PATH *) DevicePathNode)->MBRType == MBR_TYPE_EFI_PARTITION_TABLE_HEADER &&
              ...
              if (!EFI_ERROR (Status)) {
                ...
                // Measure GPT disk.
                //
                Status = Tcg2MeasureGptTable (mTcg2Protocol, Handle);
                DEBUG ((EFI_D_ERROR, "DxeTpm2MeasureBootHandler - Tcg2MeasureGptTable - %r\n", Status));
                if (!EFI_ERROR (Status)) {
                  //
                  // GPT disk check done.
                  //
    ...
    if ((!ApplicationRequired) ||
        ...
        DEBUG ((EFI_D_ERROR, "DxeTpm2MeasureBootHandler - Tcg2MeasurePeImage - %r\n", Status));
    }
  ...
  )
{
    ...
    Status = mTcg2Protocol->GetCapability(mTcg2Protocol, &ProtocolCapability);
    if (EFI_ERROR (Status)) {
        DEBUG ((EFI_D_ERROR, "TCG2 GetCapability FAILED %r\n", Status));
        return EFI_SUCCESS;
    }
    if (ProtocolCapability.TPMPresentFlag == FALSE) {
        DEBUG ((EFI_D_INFO, "TPM device not present.\n"));
        return EFI_SUCCESS;
    }
